/**
 * ============================================================================
 * SCRIPT 3: MENSUALES NIBOL MULTIMARCAS
 * CyclicStock PRO - Google Apps Script Connector (Inventario Mensual / Cierre General)
 * ============================================================================
 * Archivo en Google Drive: "MENSUALES NIBOL MULTIMARCAS" (Carpeta Nibol/ciclicos)
 * 
...
var INVENTORY_TYPE_NAME = "INVENTARIO MENSUAL";
var INVENTORY_FILE_NAME = "MENSUALES NIBOL MULTIMARCAS";

// ----------------------------------------------------------------------------
// Manejador GET: Consultas de Inventario Mensual, Hojas y Métricas
// ----------------------------------------------------------------------------
function doGet(e) {
  ...
  try {
    ...
    if (action === 'getSheets') {
      ...
      return jsonResponse({ success: true, inventoryType: 'mensual', sheets: sheets });
    }
    ...
    if (action === 'getAnalytics') {
      ...
      return jsonResponse({ success: true, inventoryType: 'mensual', centro: String(centro), ...analytics });
    }
...
function exportConcludedCycleToDrive(ss, data) {
  ...
  var fileName = dateStr + '_Mensual_Centro_' + centro;
  ...
  exportCentroAuditSheet(ss, newSs, centro, nowFull, opName, '[CIERRE CONCLUIDO] ' + (data.notes || 'Conteo mensual finalizado y firmado'));
...
function exportJustifiedCycleToDrive(ss, data) {
  ...
  var dateStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  var nowFull = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss');
  var fileName = data.nuevoNombre || (dateStr + '_Mensual_Centro_' + centro + '_Revisado');
